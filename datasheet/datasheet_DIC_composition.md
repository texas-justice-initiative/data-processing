...
The dataset includes all deaths in custody (a "custodial death" is a death in jail, prison, custody, or the process of arrest) in Texas since 2005, as reported to the Office of the Attorney General. As of 02/01/2020, there are 9980 death records in the dataset. Each row of the dataset represents a deceased person who died in custody in Texas. Currently, 5 custody types exist: `['JAIL - COUNTY' 'POLICE CUSTODY (PRE-BOOKING)' 'PRIVATE FACILITY'
 'PRISON' 'JAIL - MUNICIPAL']` 

The dataset has 51 columns.

|                                                | Data Types   | Suggested Data Types   |   No. Missing Values |
|:-----------------------------------------------|:-------------|:-----------------------|---------------------:|
| record_id                                      | object       | str                    |                    0 |
| num_revisions                                  | float64      | int                    |                 2531 |
| form_version                                   | object       | str                    |                    0 |
| report_date                                    | object       | np.datetime64          |                  148 |
| date_time_of_custody_or_incident               | object       | np.datetime64          |                  151 |
| name_first                                     | object       | str                    |                    0 |
| name_last                                      | object       | str                    |                    0 |
| name_middle                                    | object       | str                    |                 4446 |
| name_suffix                                    | object       | str                    |                 9486 |
| name_full                                      | object       | str                    |                    0 |
| date_of_birth                                  | object       | np.datetime64          |                  154 |
| age_at_time_of_death                           | float64      | int                    |                    8 |
| sex                                            | object       | str                    |                    0 |
| race                                           | object       | str                    |                   37 |
| death_date                                     | object       | np.datetime64          |                    0 |
| death_date_and_time                            | object       | np.datetime64          |                    0 |
| death_location_county                          | object       | str                    |                 2519 |
| death_location_city                            | object       | str                    |                  155 |
| death_location_street_address                  | object       | str                    |                 2521 |
| death_location_type                            | object       | str                    |                 2519 |
| death_location_type_other                      | object       | str                    |                 9839 |
| death_from_pre_existing_medical_condition      | object       | str                    |                  149 |
| manner_of_death                                | object       | str                    |                    0 |
| manner_of_death_description                    | object       | str                    |                 4412 |
| means_of_death                                 | object       | str                    |                 2519 |
| means_of_death_other                           | object       | str                    |                 9291 |
| medical_cause_of_death                         | object       | str                    |                 2528 |
| medical_examinor_coroner_evalution             | object       | str                    |                  152 |
| medical_treatment                              | object       | str                    |                  148 |
| days_from_custody_to_death                     | float64      | int                    |                  159 |
| who_caused_death_in_homicide_or_accident       | object       | str                    |                    4 |
| who_caused_death_in_homicide_or_accident_other | object       | str                    |                 9933 |
| offense_1                                      | object       | str                    |                  164 |
| offense_2                                      | object       | str                    |                 5679 |
| offense_3                                      | object       | str                    |                 7009 |
| type_of_offense                                | object       | str                    |                 5254 |
| type_of_offense_other                          | object       | str                    |                 8930 |
| were_the_charges                               | object       | str                    |                  147 |
| facility_entry_date_time                       | object       | np.datetime64          |                 4584 |
| type_of_custody                                | object       | str                    |                    0 |
| specific_type_of_custody_facility              | object       | str                    |                  280 |
| agency_address                                 | object       | str                    |                 2519 |
| agency_city                                    | object       | str                    |                 2519 |
| agency_county                                  | object       | str                    |                   19 |
| agency_name                                    | object       | str                    |                    0 |
| agency_zip                                     | float64      | int                    |                 2520 |
| entry_behavior                                 | object       | str                    |                 9255 |
| other_behavior                                 | object       | str                    |                 9805 |
| exhibit_any_medical_problems                   | object       | str                    |                 7900 |
| exhibit_any_mental_health_problems             | object       | str                    |                 7930 |
| make_suicidal_statements                       | object       | str                    |                 7932 |

As of 2020/03/08, there are 9980 death records in the dataset and each row has 51 attributes (columns). See above.

# Completeness and sampling bias 

## Does the dataset contain all possible instances or is it a sample from a larger set? 

The dataset has a sampling bias. According to the [Texas Sunset Advisory Commission Report on Jail Standards](https://www.sunset.texas.gov/public/uploads/files/reports/Texas%20Commission%20on%20Jail%20Standards%20SER.pdf):
...
The dataset has missing information. The number of missing rows per column is summarized in the table above. Some columns have both 'Unknown' and missing values. These are different. In the custodial death report, certain questions have 'Unknown' as a checkbox. Thus, 'Unknown' means this box has been checked and a missing value means the question was not answered at all. Sometimes OAG will release amended reports after certain investigations to address missing values. In this case, the amended reports will overwrite the old ones.
...
The dataset is self-contained but it is based on [the raw OAG reports](https://oagtx.force.com/cdr/cdrreportdeaths).
...
The dataset identifies subpopulations by age (`age_at_time_of_death`), gender (`sex`, binary), and race (`race`: ['OTHER' 'HISPANIC' 'BLACK' 'WHITE' nan]). Their respective distributions are the following:
